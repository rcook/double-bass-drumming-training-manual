# R&L Ch. 20 — Skiplets

- **Stage:** 6 (Rondinelli additions, sixth chapter)
- **Book pages:** R&L pp. 92–95
- **Total exercises:** ~58 — Warm-Ups 1–8 (each with footing variants), Beats 1–28, Fills 1–22
- **Section labels present:** Warm-Ups, Beats, Fills

**Session role:** focus block.

Every session opens with warm-up on the current R&L Ch. 1 exercises before you work this chapter. Baseline 30-min session: ~7–8 min warm-up (Ch. 1) → ~15 min focus (this chapter) → ~5–6 min application → ~2 min cool-down. Full detail in `approach.md` §6.

## How to use this sheet

Sixth chapter of Stage 6 (`approach.md` §5). R&L defines a *skiplet* precisely: a triplet where one foot plays the first partial and the other foot plays the next two — LRR / RLL and similar. Any combination is allowed; skiplets can start or end with the single-foot side. The Warm-Ups walk through the footing variants; the Beats and Fills put them into grooves and licks.

R&L is explicit about two things. First, try all possible footings in the warm-ups — the point is to discover which skiplets suit your style, and that only shows up once you have played several. Second, feel free to substitute your preferred skiplet footing when working the beats and fills; the printed footings are examples, not prescriptions.

R&L also notes an orchestration option: skiplets can be played with the left foot doubling the hi-hat pedal together with the left bass drum pedal. That is a fun texture; it is not required.

The bpm defaults come from the triplet-material row of `approach.md` §4 (start 40 bpm, target ~110 bpm).

## Core (most sessions)

| Ex. | Section | Role | Start | Target |
| --- | --- | --- | --- | --- |
| 1 | Warm-Ups | LRR / RLL skiplet (entry point) | 40 bpm | 110 bpm |
| 3 | Warm-Ups | Skiplet variant, RLR-RLR pattern | 40 bpm | 110 bpm |
| 1 | Beats | First skiplet beat | 40 bpm | 110 bpm |
| 5 | Beats | Skiplet beat variant | 40 bpm | 110 bpm |
| 1 | Fills | First skiplet fill | 40 bpm | 110 bpm |

## Rotation (some sessions)

| Ex. | Section | Role | Start | Target |
| --- | --- | --- | --- | --- |
| 5 | Warm-Ups | Skiplet footing variant | 40 bpm | 110 bpm |
| 7 | Warm-Ups | Skiplet footing variant | 40 bpm | 110 bpm |
| 13 | Beats | Skiplet beat variant | 40 bpm | 110 bpm |
| 5 | Fills | Skiplet fill variant | 40 bpm | 110 bpm |

## Reference (discretionary)

Warm-Ups 2, 4, 6, 8 are footing pairs to their odd-numbered neighbours. Beats 2–4, 6–12, 14–28 and Fills 2–4, 6–22 are further skiplet patterns. R&L's own advice to substitute the footings you prefer applies here — sample rather than checklist.

## Practice notes

- Skiplets are footing-driven, not rhythm-driven. R&L is explicit that you should try every possible footing in the warm-ups to find which ones suit your style; do not just default to the first printed footing.
- Substitute skiplet footings freely in the beats and fills once the warm-ups are stable. The chapter rewards experimentation.
- The left-foot-on-hi-hat orchestration is optional. Try it once the base skiplet feel is solid.
- Placement of the double side matters — the two-in-a-row footprint tends to rush against the single-foot side. Count the triplet subdivision aloud if it starts drifting.

## Log line format

Log this chapter's work with the exercise number prefixed by `20.` and the section identifier — for example `20.w1  60 bpm  pass  LRR` for Warm-Ups exercise 1, `20.b1  56 bpm  near  drifting` for Beats exercise 1, or `20.f1  56 bpm  pass  clean` for Fills exercise 1. See `approach.md` §7.
