...
**Note on Ex. 1 (edition discrepancy).** The Revised Edition of R&L prints bar 2 of Ex. 1 on the same bass drum as bar 1—both on BD I (strong foot). This appears to be a printing error. The Original Edition places bar 2 on BD II (weak foot), which fits the chapter's stated purpose ("isolate and help develop your weak foot") and reads as a strong-foot / weak-foot alternation across the two bars. Play it as the Original Edition notates: bar 1 strong foot, bar 2 weak foot. See [approach.md §3.2](approach.md#32-original-edition-compatibility) for the general edition policy and the full list of confirmed differences between the two editions.

## Rotation (some sessions)

Once the Core is stable (say, two weeks of consistent `pass` verdicts on exercises 1, 10 and 11), you can start adding a Rotation exercise. Add *one* of the two below on a given session; do not add both on the same day. Concretely: pick one row per session, and treat it as the fourth exercise in the warm-up block—look up bpm, set metronome, set timer for ~2 min, play, log. Alternate between Ex. 12 and Ex. 13 across sessions so both get worked over time.

| Ex. | Pattern | Start | Target |
| --- | --- | --- | --- |
| 12 | Sextuplet single-stroke roll between feet | 50 bpm | 100 bpm |
| 13 | 32nd-note single-stroke roll between feet | 40 bpm | 60 bpm |
