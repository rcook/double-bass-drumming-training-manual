...
- **`current bpm`** ← follows one of two rules depending on the exercise's prior history. This is the key place the working state and the log diverge, so both cases are spelt out:
  ...
  Note: `ceiling` is the session's highest cleanly-passed bpm (per [§7 step 6](#focus-block)), *not* the `current bpm` you started the session at. If you started at 60 and probed cleanly to 66, this session's ceiling is 66 — the 60 does not enter the formula. Worked example: session A ends with ceiling 66; session B starts at 60, probes cleanly to 66, ceiling 66; `current bpm` after session B = min(66, 66) + one increment = 68.
