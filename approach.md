...
7. Then update the per-chapter working state per [§8](#8-tracking) — set `last pass bpm` to the session ceiling and advance `current bpm` per the rule spelled out in §8. If the session's starting bpm was `near` or `fail`, apply the [§4](#4-the-tempo-target-and-clean-pass-rule) regression rule instead; you tried and learned.
...
- **`last pass bpm`** ← this session's ceiling (the highest bpm at which you had a clean `pass`; the same value that goes in the log entry's bpm field).
- **`current bpm`** ← one increment past the highest bpm cleared cleanly in *both* this session and the previous session — concretely, `min(this session's ceiling, previous session's ceiling) + one increment`. On a first-ever session for an exercise (no previous session yet), set `current bpm` to this session's ceiling directly — you have one clean pass at that tempo, waiting for confirmation next session. Under **hold-only** sessions (see [§7 focus block](#focus-block) — no within-session probing; R&L Ch. 1 works this way) the two ceilings match and `current bpm` advances every second session; under **hold-and-probe** (Ch. 2 onward) they can diverge, and `current bpm` trails the ceiling by one increment because the two-pass rule ([§4](#4-the-tempo-target-and-clean-pass-rule)) requires a bpm to be cleared in two consecutive sessions before the working state commits to it.
- **`last pass date`** ← today.
- **Regression:** if the session's starting bpm was `near` or `fail`, apply the [§4](#4-the-tempo-target-and-clean-pass-rule) regression rule (drop `current bpm` by one increment) and leave `last pass bpm` unchanged.
