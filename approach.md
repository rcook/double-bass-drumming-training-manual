...
4. If the current bpm was clean, raise the metronome by one increment (+2 to +4 bpm per [§4](#4-the-tempo-target-and-clean-pass-rule)) and play for another minute to *probe* the next step. If the current bpm was not clean, keep the current bpm and use the remaining time to reinforce it, not to escape it. **If a probe fails with time still on the clock, drop the metronome back to the last cleanly-passed rung and use the remaining time to reinforce it—do not grind at a bpm you cannot hold.** Sustained reps above the session ceiling groove tension and uneven mechanics without moving the ceiling; the session ceiling is already recorded, and quality reps at the rung below it are the better use of the remaining time.
...
- **`current bpm`** ← follows one of two rules depending on the exercise's prior history. This is the key place the working state and the log diverge, so both cases are spelled out:
